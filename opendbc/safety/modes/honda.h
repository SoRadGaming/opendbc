...
static safety_config honda_nidec_init(uint16_t param) {
  ...
  static CanMsg HONDA_N_TX_MSGS[] = {{0xE4, 0, 5, .check_relay = true}, {0x194, 0, 4, .check_relay = true}, {0x1FA, 0, 8, .check_relay = false},
                                     ...
                                     {0x17C, 2, 8, .check_relay = false}};  // OP re-sends POWERTRAIN_DATA on bus 2 w/ ACC_STATUS=0 (stock ACC stand-down)
...
static bool honda_nidec_fwd_hook(int bus_num, int addr) {
  ...
  // block stock POWERTRAIN_DATA to the ACC radar; OP re-sends it on bus 2 with ACC_STATUS=0
  if ((bus_num == 0) && (addr == 0x17C)) {
    block_msg = true;
  }
